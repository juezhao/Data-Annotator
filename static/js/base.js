...
const session = {
    ...
    validateCookies(){
        ...
        if (!cookieEnabled){ 
            document.cookie = 'DataAnnotator-test-cookie';
            cookieEnabled = document.cookie.indexOf('DataAnnotator-test-cookie') != -1;
        }
        return cookieEnabled || this.throwCookieError();
    },
    
    throwCookieError(){
        alert('Cookies must be enabled to use DataAnnotator.');
        window.location = '/';
    }
}
